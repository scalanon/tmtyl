...
import org.scalanon.tmtyl.game.Score
...
object ScoreIO {
  def loadScore(): Unit =
    try {
      ...
      val mobile = Tmtyl.mobile
      get.setUrl(s"$URL?mobile=$mobile")
  ...
  val URL = "https://api.scalanon.org/tmtyl/score"

  val HMAC_SHA256 = "HmacSHA256"

  def signPayload(payload: Map[String, String]): Map[String, String] = {
    val mac = Mac.getInstance(HMAC_SHA256)
    mac.init(new SecretKeySpec(Tmtyl.key.getBytes("UTF-8"), HMAC_SHA256))
    val plaintext =
...
class ScoreListener extends IOListener("loadScore") {
  override def handleHttpResponse(httpResponse: Net.HttpResponse): Unit =
    httpResponse.getResultAsString match {
      case ScoreRE(score) =>
        Tmtyl.globalHigh = score.toInt
      case _ =>
    }

  final val ScoreRE = "^([0-9]+)$".r
}
